...
def build_sim_args(frames: int, case_seed: int, rates: list[float], trace_prefix: Path | None) -> str:
    sim_args = [
        f"+SWB_FRAMES={frames}",
...
def main() -> int:
    ...
    for run_id, (rates, case_seed) in enumerate(cases):
        ...
        sim_args = build_sim_args(args.frames, case_seed, rates, None)
        rc = run_make(
            uvm_dir,
            [
                "run",
        ...
        if not run_passed and args.trace_failures:
            ...
            trace_args = build_sim_args(args.frames, case_seed, rates, trace_prefix)
            run_make(
                uvm_dir,
                [
                    "run",
