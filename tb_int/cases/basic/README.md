...
5. If you want to split the OPQ seam, run `make ip-plain-basic-2env REPLAY_DIR=$(pwd)/tb_int/cases/basic/ref/out`.
6. When the full Mentor verification runtime is installed, run `make ip-uvm-basic SIM_ARGS="+SWB_REPLAY_DIR=$(pwd)/tb_int/cases/basic/ref/out"` to replay the exact same traffic in UVM/RTL.
...
- `plain_2env/` is the split workaround path: VHDL-only post-OPQ datapath plus a DPI-backed OPQ seam harness.
...
Use `ref/` for the current no-runtime workaround, `plain/` for the low-license mixed-language path, `plain_2env/` for the split OPQ-seam workaround, and `uvm/` for the full harness.
