...
- [`cases/cross/ghdl/`](cases/cross/ghdl/) — lightweight GHDL packet-evidence fixture with decoded GTKWave packets, OPQ dataflow signals, and named checkpoint validation.
...
14. `make ip-ghdl-cross-run` — fast deterministic packet-evidence GHDL waveform run.
...
| `make ip-opq-csr-*` | OPQ JTAG Avalon master CSR probe/dump/write/lane-mask/monitor helpers; see [`../docs/registers/opq_jtag_csr.md`](../docs/registers/opq_jtag_csr.md) |
...
| `make ip-ghdl-cross-objects` | compile the lightweight packet-evidence GHDL waveform fixture |
| `make ip-ghdl-cross-run` | run the GHDL fixture and write the VCD/log/GTKWave save file under `cases/cross/ghdl/report/` |
| `make ip-ghdl-cross-gtkw` | regenerate the packet-decoded GTKWave save file and translate filters |
...
- The OPQ Qsys wrapper includes an internal JTAG Avalon master connected to the OPQ CSR slave. The SVD-backed live-hardware path is documented in [`../docs/registers/opq_jtag_csr.md`](../docs/registers/opq_jtag_csr.md); dry-run validation confirms the generated SVD currently exposes 79 registers including UID/META, global frame-table counters, and four per-lane counter/control blocks.
...
- The GHDL cross fixture is green for the packet-evidence shape: five scenarios, at least three UVM-spaced frames per scenario, runtime lane-mask and CSR read events, 12 named checkpoints, and 45 VCD signal expectations. Its GTKWave view is grouped clock/reset -> runtime controls -> four RX packet lanes -> OPQ egress packet -> OPQ internals/fill levels -> DMA sample -> appendix `A0` diagnostics -> appendix `A1` counters -> appendix `A2` CSR map/read bus.
- For exact native-SV timing and content correlation, `cases/cross/ghdl/README.md` documents the `export_wave_case_bundle.py --opq-source-mode native_sv_signoff`, `UVM_B001_VCD=... make -C cases/cross/ghdl run checkpoints`, and `compare_opq_wave_timing.py` flow. That path extracts UVM `feb_if0..3` and `opq_if` packets into the GHDL replay, then checks SOP/EOP timing, frame timestamp steps, OPQ count fields, packet grammar, and ingress-to-OPQ hit/subframe-hit multisets.
