...
Every `anchored_hybrid` and above saves one isolated UCDB plus checkpoint UCDBs per log-spaced milestone under [`report/signoff/txn_growth/`](../report/signoff/txn_growth/). `bucket_frame` and `all_buckets_frame` save one merged UCDB under [`report/signoff/cross/`](../report/signoff/cross/).
...
- **`checkpoint_soak`** — cadence `1, 2, 4, 8, …` frames; each checkpoint saves a UCDB under [`report/signoff/txn_growth/<case_id>/`](../report/signoff/txn_growth/).
