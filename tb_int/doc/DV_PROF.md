...
- **checkpoint UCDB** — log-spaced save cadence `1, 2, 4, 8, 16, … ≤ total_frames` under [`report/signoff/txn_growth/`](../report/signoff/txn_growth/). Applies to `checkpoint_soak` and seeded long runs.
...
| P026 | R | live UVM | 1024-frame soak `λ=0.25`, seed `260421` | checkpoints up to 1024; coverage growth curve logged in `report/signoff/txn_growth/P026.md` | all + ckp |
...
| P091 | R | live UVM | checkpoint UCDB cadence `1,2,4,…,4096` on 4096-frame run, `λ=0.30` | coverage growth curve published in `report/signoff/txn_growth/P091.md` | all + ckp |
...
- **isolated** — per-case `make ip-uvm-basic` or `make ip-plain-basic-2env` with explicit plusargs; UCDB saved under `report/signoff/cases/P0xx.md` evidence.
