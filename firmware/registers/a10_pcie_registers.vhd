...
package a10_pcie_registers is
    ...
    constant RESET_REGISTER_W                               :  integer := 16#01#; -- DOC: Reset Register | ALL
        ...
        constant RESET_BIT_SWB_COUNTERS                         :  integer := 29; -- DOC: Reset bit for the counters on the SWB | SWB
    ...
    constant SWB_DATA_TYPE_REGISTER_W                       : integer := 16#04#; -- DOC: set the different data types (6bits) for the data paths on the SWB board | SWB
    ...
    constant SWB_GENERIC_MASK_REGISTER_W                    : integer := 16#0F#; -- DOC: Mask generic links for dev setup | SWB
    ...
    constant SWB_SUBHEAD_SUPPRESS_REGISTER_W                : integer := 16#19#; -- DOC: subheader suppression per link | SWB
    constant SWB_HEAD_SUPPRESS_REGISTER_W                   : integer := 16#1A#; -- DOC: header suppression per link | SWB
    constant SWB_ZERO_HISTOS_REGISTER_W                     : integer := 16#1B#; -- DOC: todo | SWB
    constant SWB_HISTO_ADDR_REGISTER_W                      : integer := 16#1C#; -- DOC: todo | SWB
    constant SWB_HISTO_CHIP_SELECT_REGISTER_W               : integer := 16#1D#; -- DOC: todo | SWB
    constant SWB_HISTO_LINK_SELECT_REGISTER_W               : integer := 16#1E#; -- DOC: todo | SWB
    constant SWB_LOOKUP_CTRL_REGISTER_W                     : integer := 16#1F#; -- DOC: ctrl register to write chip lookup | SWB
    constant SWB_LOOKUP_DS_CTRL_REGISTER_W                  : integer := 16#20#; -- DOC: ctrl register to write chip lookup | SWB
        subtype SWB_LOOKUP_CTRL_ADDR_RANGE                      is integer range 6 downto 0;      -- DOC: addr for the RAM | SWB
        subtype SWB_LOOKUP_CTRL_COMMAND_RANGE                   is integer range 8 downto 7;      -- DOC: command for the state machine | SWB
        subtype SWB_LOOKUP_CTRL_VALUE_RANGE                     is integer range 22 downto 9;     -- DOC: lookup value | SWB

    constant DDR_CONTROL_W                                  : integer := 16#21#; -- DOC: Control register for the ddr_memory_controller | FARM
    ...
    constant FARM_LINK_MASK_REGISTER_W                      : integer := 16#22#; -- DOC: Mask Farm links | FARM

    constant DATA_REQ_A_W                                   : integer := 16#23#; -- DOC: Register for requesting subheaders from DDR, for SUBTS in GPU_SUBTS_SEL DO writereg(SUBTS) | FARM
    constant DATA_REQ_B_W                                   : integer := 16#24#; -- DOC: Register for requesting subheaders from DDR, for SUBTS in GPU_SUBTS_SEL DO writereg(SUBTS) | FARM
    constant DATA_TSBLOCK_DONE_W                            : integer := 16#25#; -- DOC: dynamic limit when we change from writing to reading (15 downto 8 from 35 downto 4 of the 48b TS) | FARM
    constant FARM_ID_REGISTER_W                             : integer := 16#26#; -- DOC: Farm ID written to the reserved filed of the MIDAS bank | FARM
    constant FARM_REQ_EVENTS_W                              : integer := 16#27#; -- DOC: total number of requested events (should match to len(DATA_REQ_A/B_W) from GPU selection | FARM
    constant FARM_CTL_REGISTER_W                            : integer := 16#28#;
        constant USE_BIT_PIXEL_ONLY                             : integer := 0;
        constant USE_BIT_SCIFI_ONLY                             : integer := 1;

    constant RESET_LINK_CTL_REGISTER_W                      : integer := 16#29#;
        subtype  RESET_LINK_COMMAND_RANGE                       is integer range 7 downto 0;
        subtype  RESET_LINK_FEB_RANGE                           is integer range 31 downto 29;
    constant RESET_LINK_RUN_NUMBER_REGISTER_W               : integer := 16#2A#;
    constant FARM_EVENT_ID_REGISTER_W                       : integer := 16#2B#; -- DOC: set MIDAS event id (15 downto 0) and trigger mask (31 downto 16) | FARM
    constant FARM_GPU_EVENT_SIZE_W                          : integer := 16#2C#; -- DOC: set the size of the GPU event (default: 166666.67 x 96b = 2MB | 0x28B0A x 96b + 64b = 2MB) | FARM
    constant FARM_GPU_EVENT_PADDING_W                       : integer := 16#2D#; -- DOC: set the padding size in the GPU event (default: 1000=0x3E8) | FARM
    constant FARM_SERIAL_NUMBER_W                           : integer := 16#2E#; -- DOC: set the start serial number which will increase by 1 (default: 0) | SWB | FARM
    constant XCVR_CTRL_REGISTER_W                          : integer := 16#2F#; -- DOC: select internal xcvr-register | SWB | FARM
        subtype XCVR_CTRL_CH_RANGE                              is integer range 21 downto 16; -- DOC: xcvr channel selector | SWB | FARM
        subtype XCVR_CTRL_REG_RANGE                             is integer range 7 downto 0; -- DOC: xcvr register selector | SWB | FARM
    ...
    constant DMA_REGISTER_W                                 : integer := 16#38#;
        ...
        constant FLUSH_BIT_ENABLE                           : integer := 20;
        subtype FLUSH_TEST_RANGE                            is integer range 31 downto 21;
    ...
    constant MUSIP_MUX_ERROR_REGISTER_R                     : integer := 16#0D#;
    ...
    constant EVENT_BUILD_IDLE_NOT_HEADER_R                  : integer := 16#1D#;
    constant EVENT_BUILD_SKIP_EVENT_DMA_R                   : integer := 16#1E#;
    constant EVENT_BUILD_CNT_EVENT_DMA_R                    : integer := 16#1F#;
    constant EVENT_BUILD_TAG_FIFO_FULL_R                    : integer := 16#20#;
    ...
    constant XCVR_CTRL_REGISTER_R                           : integer := 16#2F#;
    ...
    constant SWB_LINK_COUNTER_REGISTER_R                    : integer := 16#34#;
