...
architecture rtl of frame_rcv is
    ...
    signal p_state_wait_cnt, n_state_wait_cnt : integer range 0 to 7; -- cnt to wait withiin state
...
--0b110 Channel event counter data trans
    ...
    begin
        ...
        if ( i_byteisk = '1' and i_data = x"BC" ) then
            ...
            when FS_EVENT_COUNTER =>
                ...
                if ( p_state_wait_cnt = 2 ) then
                    ...
                    if ( p_frame_len(9 downto 8) = "00" and i_data = 0 ) then
                        n_state             <= FS_CRC_CALC;
                        n_state_wait_cnt    <= 2;
                    else
                        n_state <= FS_UNPACK;
        ...
        -- else
        --   n_state <= FS_IDLE;
        --   n_crc_err_count <= p_crc_err_count + 1;
        -- end if;
                end if;
