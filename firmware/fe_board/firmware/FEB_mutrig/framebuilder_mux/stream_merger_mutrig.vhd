...
-- [AK] TODO: use stream_merger with sop/eop = '1'
...
begin
    ...
    if ( i_reset_n /= '1' ) then
        ...
        if ( or_reduce(i_rempty and index) = '0' and i_wfull = '0' ) then
            -- write data
            o_we <= '1';

            if ( or_reduce(sop and index) = '1' ) then
                -- set SOP mark
                busy <= '1';
            elsif ( or_reduce(index) = '1' ) then
                -- reset SOP mark
                busy <= '0';
                -- go to next index
