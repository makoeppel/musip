...
begin
    ...
    fsm_comb :
    process(p_state, s_cnt, i_fast, i_enable, p_wait_cnt, p_event_cnt, p_frame_number, p_event_data)
    begin
        n_state <= p_state;

        n_event_cnt <= p_event_cnt;
        n_wait_cnt  <= p_wait_cnt - 1;
        ...
        when fs_new_frame =>
            n_wait_cnt <= p_wait_cnt - 1;
            if p_wait_cnt = 2 then
                n_new_frame <= '1';
            end if;
            if p_wait_cnt = 0 then
                n_frame_info_rdy <= '1';
                if i_fast = '0' then
        ...
        when fs_new_word =>
            if p_wait_cnt = 0 then
                n_event_data  <= p_event_data + 1;
                n_event_cnt   <= p_event_cnt - 1;
                if p_event_cnt = 0 then
        ...
        when fs_end_of_frame =>
            ...
            if p_wait_cnt = 0 then
                n_frame_number <= p_frame_number + 1;
                n_wait_cnt <= std_logic_vector(to_unsigned(255 - 6 * to_integer(unsigned(i_cnt)), 11));
                n_state <= fs_wait;
            end if;
