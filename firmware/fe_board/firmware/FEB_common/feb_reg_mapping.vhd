...
port (
    ...
    i_reset_phase               : in    std_logic_vector(31 downto 0) := x"CCCCCCCC";
    ...
    o_reset_delay               : out   std_logic_vector(7 downto 0);
...
-- outputs
    ...
    signal reg_reset_delay          : std_logic_vector(7 downto 0);
...
begin
    ...
    if ( i_reset_n = '0' ) then
        ...
        -- Fireflies
        if ( regaddr = FIREFLY_STATUS_REGISTER_R + FIREFLY1_INDEX_TEMP and i_reg_re = '1' ) then
            o_reg_rdata <= x"000000" & i_ffly_temp(7 downto 0);
        end if;
        if ( regaddr = FIREFLY_STATUS_REGISTER_R + FIREFLY2_INDEX_TEMP and i_reg_re = '1' ) then
            o_reg_rdata <= x"000000" & i_ffly_temp(15 downto 8);
        end if;
        if ( regaddr = FIREFLY_STATUS_REGISTER_R + FIREFLY1_INDEX_VOLT and i_reg_re = '1' ) then
            o_reg_rdata <= x"0000" & i_ffly_vcc(15 downto 0);
        end if;
        if ( regaddr = FIREFLY_STATUS_REGISTER_R + FIREFLY2_INDEX_VOLT and i_reg_re = '1' ) then
            o_reg_rdata <= x"0000" & i_ffly_vcc(31 downto 16);
        end if;
        if ( regaddr = FIREFLY_STATUS_REGISTER_R + FIREFLY1_INDEX_RX1_POW and i_reg_re = '1' ) then
            o_reg_rdata <= x"0000" & i_ffly_pwr(15 downto 0);
        end if;
        if ( regaddr = FIREFLY_STATUS_REGISTER_R + FIREFLY1_INDEX_RX2_POW and i_reg_re = '1' ) then
            o_reg_rdata <= x"0000" & i_ffly_pwr(31 downto 16);
        end if;
        if ( regaddr = FIREFLY_STATUS_REGISTER_R + FIREFLY1_INDEX_RX3_POW and i_reg_re = '1' ) then
            o_reg_rdata <= x"0000" & i_ffly_pwr(47 downto 32);
        end if;
        if ( regaddr = FIREFLY_STATUS_REGISTER_R + FIREFLY1_INDEX_RX4_POW and i_reg_re = '1' ) then
            o_reg_rdata <= x"0000" & i_ffly_pwr(63 downto 48);
        end if;
        if ( regaddr = FIREFLY_STATUS_REGISTER_R + FIREFLY2_INDEX_RX1_POW and i_reg_re = '1' ) then
            o_reg_rdata <= x"0000" & i_ffly_pwr(79 downto 64);
        end if;
        if ( regaddr = FIREFLY_STATUS_REGISTER_R + FIREFLY2_INDEX_RX2_POW and i_reg_re = '1' ) then
            o_reg_rdata <= x"0000" & i_ffly_pwr(95 downto 80);
        end if;
        if ( regaddr = FIREFLY_STATUS_REGISTER_R + FIREFLY2_INDEX_RX3_POW and i_reg_re = '1' ) then
            o_reg_rdata <= x"0000" & i_ffly_pwr(111 downto 96);
        end if;
        if ( regaddr = FIREFLY_STATUS_REGISTER_R + FIREFLY2_INDEX_RX4_POW and i_reg_re = '1' ) then
            o_reg_rdata <= x"0000" & i_ffly_pwr(127 downto 112);
        end if;
        if ( regaddr = FIREFLY_STATUS_REGISTER_R + FIREFLY1_INDEX_ALARM and i_reg_re = '1' ) then
            o_reg_rdata <= i_ffly_alarm(31 downto 0);
        end if;
        if ( regaddr = FIREFLY_STATUS_REGISTER_R + FIREFLY2_INDEX_ALARM and i_reg_re = '1' ) then
        ...
        -- reset delay
        if ( regaddr = RESET_DELAY_REGISTER_RW and i_reg_re = '1' ) then
            o_reg_rdata <= x"000000" & reg_reset_delay;
        end if;
        if ( regaddr = RESET_DELAY_REGISTER_RW and i_reg_we = '1' ) then
            reg_reset_delay <= i_reg_wdata(7 downto 0);
        end if;
