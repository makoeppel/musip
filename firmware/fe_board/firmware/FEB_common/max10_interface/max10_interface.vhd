...
port (
    ...
    o_adc_reg               : out   slv32_array_t(4 downto 0);
    ...
    i_programming_ctrl      : in    std_logic_vector(31 downto 0);
    i_programming_data      : in    std_logic_vector(31 downto 0);
    i_programming_data_ena  : in    std_logic;
    i_programming_addr      : in    std_logic_vector(31 downto 0);
    i_programming_addr_ena  : in    std_logic;
...
architecture rtl of max10_interface is
    ...
    type max_spi_state_t            is (
        idle, programming, fifocopy, control, controlwait, programmingaddrwait, programmingaddr,
...
begin
    ...
    port map (
        ...
        -- Interface to Arria
        i_strobe          => max_spi_strobe,
        i_addr            => max_spi_addr,
        i_rw              => max_spi_rw,
        i_data_to_max     => max_spi_data_to_max,
        i_numbytes        => max_spi_numbytes,
        o_next_data       => max_spi_next_data,
        o_word_from_max   => max_spi_word_from_max,
        o_word_en         => max_spi_word_en,
        o_byte_from_max   => max_spi_byte_from_max,
        o_byte_en         => max_spi_byte_en,
        o_busy            => max_spi_busy,
    ...
    program_req <= i_programming_ctrl(0);
    ...
    if ( i_reset_n = '0' ) then
        ...
        programming_addr_ena_reg <= i_programming_addr_ena;
        ...
        when programmingaddr =>
            ...
            max_spi_strobe   <= '1';
            max_spi_rw       <= '1';
            max_spi_data_to_max     <= i_programming_addr;
        ...
        when maxadc =>
            ...
            if(max_spi_word_en = '1') then
                ...
                if(wordcounter = 0) then
                    o_adc_reg(0)    <= max_spi_word_from_max;
                elsif(wordcounter = 1) then
                    o_adc_reg(1)    <= max_spi_word_from_max;
                elsif(wordcounter = 2) then
                    o_adc_reg(2)    <= max_spi_word_from_max;
                elsif(wordcounter = 3) then
                    o_adc_reg(3)    <= max_spi_word_from_max;
                elsif(wordcounter = 4) then
                    o_adc_reg(4)    <= max_spi_word_from_max;
                    max_spi_strobe  <= '0';
    ...
    generic map (
        g_DATA_WIDTH => i_programming_data'length,
        g_ADDR_WIDTH => 9,
        g_RAM_OUTREG => "ON"--,
    )
    port map (
        i_we => i_programming_data_ena,
        i_wdata => i_programming_data,
        o_wfull => programming_status(3),
        i_wclk => i_clk_156,

        i_rack => read_programming_fifo,
        o_rdata => programming_data_from_fifo,
        o_rempty => programming_status(2),
        i_rclk => i_clk,

        -- TODO: add all bits to register file with the name
        i_reset_n => not i_programming_ctrl(1)--,
    );
