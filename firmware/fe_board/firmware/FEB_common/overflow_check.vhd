...
port (
    ...
    i_usedw         : in    std_logic_vector(g_FIFO_ADDR_WIDTH-1 downto 0 );
...
begin
    ...
    begin
    if ( i_reset_n /= '1' or DISABLE /= 0 ) then
        o_write_req             <= '0';
        o_wdata                 <= (others => '0');
        state                   <= forwarding;
        ...
        case state is
        when forwarding =>
            if ( i_write_req = '1' and i_wdata(33 downto 32) = MERGER_FIFO_PAKET_START_MARKER(1 downto 0) and i_usedw > (2**g_FIFO_ADDR_WIDTH)-MAX_PAKET_SIZE ) then
                state           <= throwing;
                o_write_req     <= '0';
            else
