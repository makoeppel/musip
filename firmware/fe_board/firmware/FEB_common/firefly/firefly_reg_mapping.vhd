...
port (
    ...
    o_loopback                  : out   std_logic_vector(g_CHANNELS-1 downto 0);
    o_tx_reset                  : out   std_logic_vector(g_CHANNELS-1 downto 0);
    o_rx_reset                  : out   std_logic_vector(g_CHANNELS-1 downto 0);
    o_lvds_align_reset_n        : out   std_logic;
    i_tx_status                 : in    std_logic_vector(g_CHANNELS-1 downto 0) := (others => '0');
    i_tx_errors                 : in    std_logic_vector(g_CHANNELS-1 downto 0) := (others => '0');
    i_lvds_controller_state     : in    std_logic_vector(3 downto 0);

    i_rx_ready                  : in    std_logic_vector(g_CHANNELS-1 downto 0) := (others => '0');
    i_rx_lockedtoref            : in    std_logic_vector(g_CHANNELS-1 downto 0) := (others => '0');
    i_rx_lockedtodata           : in    std_logic_vector(g_CHANNELS-1 downto 0) := (others => '0');
    i_rx_locked                 : in    std_logic_vector(g_CHANNELS-1 downto 0) := (others => '0');
    i_rx_syncstatus             : in    slv4_array_t(g_CHANNELS-1 downto 0) := (others => (others => '0'));
    i_rx_errdetect              : in    slv4_array_t(g_CHANNELS-1 downto 0) := (others => (others => '0'));
    i_rx_disperr                : in    slv4_array_t(g_CHANNELS-1 downto 0) := (others => (others => '0'));
    i_rx_fifo_error             : in    std_logic_vector(g_CHANNELS-1 downto 0) := (others => '0');
    i_lol_cnt                   : in    slv8_array_t(g_CHANNELS-1 downto 0) := (others => (others => '0'));
    i_err_cnt                   : in    slv16_array_t(g_CHANNELS-1 downto 0) := (others => (others => '0'));
    i_rx_data                   : in    slv32_array_t(g_CHANNELS-1 downto 0) := (others => (others => '0'));
    i_rx_datak                  : in    slv4_array_t(g_CHANNELS-1 downto 0) := (others => (others => '0'));
    i_gbit                      : in    slv24_array_t(g_CHANNELS-1 downto 0) := (others => (others => '0'));
...
architecture rtl of firefly_reg_mapping is
    ...
    signal channel_sel_int : integer range 0 to g_CHANNELS-1;
    signal tx_reset        : std_logic_vector(g_CHANNELS-1 downto 0);
    signal rx_reset        : std_logic_vector(g_CHANNELS-1 downto 0);
    signal loopback        : std_logic_vector(g_CHANNELS-1 downto 0);
...
begin
    ...
    if ( i_reset_n = '0' ) then
        ...
        if ( regaddr = FIREFLY_XCVR_CH_SEL_REGISTER_RW and i_reg_we = '1' ) then
            if( i_reg_wdata < g_CHANNELS ) then
        ...
        if ( regaddr = FIREFLY_XCVR_N_CH_REGISTER_R and i_reg_re = '1' ) then
            o_reg_rdata <= std_logic_vector(to_unsigned(g_CHANNELS, 32));
        end if;

        -- Channel width
        if ( regaddr = FIREFLY_XCVR_CH_WIDTH_REGISTER_R and i_reg_re = '1' ) then
            o_reg_rdata <= std_logic_vector(to_unsigned(g_CHANNEL_BITS, 32));
        end if;
        ...
        if ( regaddr = FIREFLY_XCVR_RX_STATUS_REGISTER_R and i_reg_re = '1' ) then
            ...
            o_reg_rdata(g_CHANNEL_BITS/8-1 + 8 downto 8)    <= i_rx_syncstatus(channel_sel_int);
            o_reg_rdata(31 downto 13)                       <= (others => '0');
        end if;

        -- rx_errors
        if ( regaddr = FIREFLY_XCVR_RX_ERROR_REGISTER_R and i_reg_re = '1' ) then
            o_reg_rdata(g_CHANNEL_BITS/8-1 + 0 downto 0)    <= i_rx_errdetect(channel_sel_int);
            o_reg_rdata(g_CHANNEL_BITS/8-1 + 4 downto 4)    <= i_rx_disperr(channel_sel_int);
