...
architecture arch of sc_ram is
    ...
    signal read_delay_shift_reg         : std_logic_vector(g_READ_DELAY   downto 0) := (others => '0');
    signal read_delay_shift_reg_type    : std_logic_vector(g_READ_DELAY+1 downto 0) := (others => '0'); -- 0: Arria, 1: Nios
...
begin
    ...
    if ( i_reset_n = '0' ) then
        ...
        read_delay_shift_reg        <= read_delay_shift_reg(g_READ_DELAY-1 downto 0) & '0';
        read_delay_shift_reg_type   <= read_delay_shift_reg_type(g_READ_DELAY downto 0) & '0'; -- 0: sc_rx, 1: Nios
        avs_waitrequest_prev        <= avs_waitrequest;


        if(read_delay_shift_reg_type(g_READ_DELAY+1)='1') then -- reset avs_read_sent whenever we respond to nios
            avs_read_sent                   <= '0';
        end if;
    ...
    if ( i_reset_n = '0' ) then
        ...
        if(read_delay_shift_reg(g_READ_DELAY) = '1') then
            if(read_delay_shift_reg_type(g_READ_DELAY) = '0') then --respond to Arria10
                o_ram_rvalid    <= '1';
                o_ram_rdata     <= rdata;
    ...
    avs_waitrequest <=
        '1' when ( i_ram_re = '1' -- we don't have this cycle for the nios --(TODO: check if we can do (i_ram_re='1' and not read_delay_shift_reg_type(g_READ_DELAY) = '1') safely here)
                or i_ram_we = '1' -- same
                ...
                or (or_reduce(read_delay_shift_reg_type(g_READ_DELAY downto 0)) = '1') -- nios request in pipeline, waiting for reply
