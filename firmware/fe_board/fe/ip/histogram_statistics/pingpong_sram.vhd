...
-- Revision: 1.8
--      Date: Jul 13, 2026
--      Change: Interlock host commands and per-beat issue against pending or
--              in-flight updates to the current readout bank. This preserves
--              port-A ownership for update RMW reads after an interval swap;
--              the mailbox may stall, but every returned count is committed.
-- Revision: 1.9
--      Date: Jul 13, 2026
--      Change: Predict per-bin validity through the complete in-flight RMW
--              pipeline when accepting a same-bin update. A later update can
--              be accepted before the first write sets bank_valid; without
--              this prediction its delayed read may discard already-committed
--              RAM data after the forwarding stages have retired. The ready
--              path and one-update-per-clock pipeline are unchanged.
-- =========
...
architecture rtl of pingpong_sram is
    ...
    signal hist_target_bank   : std_logic := '0';
    signal hist_target_busy   : std_logic := '0';
...
begin
    ...
    -- cleared.
    -- A command can start only after the current readout bank is quiescent.
    -- The top-level bank ledger may retire an item when it enters this SRAM,
    -- so every internal RMW stage is checked here as an independent safety net.
    hist_target_bank <= not active_bank when i_enable_pingpong = '1' else active_bank;
    hist_target_busy <= '1' when
        (((hist_target_bank = '0') and (i_bank_pending(0) = '1')) or
         ((hist_target_bank = '1') and (i_bank_pending(1) = '1')) or
         ((clear_active = '1') and (clear_bank = hist_target_bank)) or
         ((i_upd_valid = '1') and (i_upd_bank = hist_target_bank)) or
         ((upd_issue_valid = '1') and (upd_issue_bank = hist_target_bank)) or
         ((upd_read_valid = '1') and (upd_read_bank = hist_target_bank)) or
         ((upd_add_valid = '1') and (upd_add_bank = hist_target_bank)) or
         ((upd_sum_valid = '1') and (upd_sum_bank = hist_target_bank)) or
         ((upd_write_valid = '1') and (upd_write_bank = hist_target_bank)))
        else '0';

    hist_waitrequest <= '1' when (burst_active = '1') or (hist_target_busy = '1')
                        else '0';
    ...
    begin
        if rising_edge(i_clk) then
            ...
            if i_rst = '1' then
                ...
                -- Command acceptance is interlocked by hist_waitrequest. The
                -- master holds read/address/burstcount stable while either the
                -- single burst slot is occupied or the live readout bank still
                -- has a pending/in-flight update.
                ...
                if (burst_active = '1') and (not ram_v_read_inflight) and
                   ((i_enable_pingpong = '0') or (hist_target_busy = '0')) then
                    ram_v_hist_issue := '1';
                ...
                if ram_v_accept_update then
                    ...
                    -- bank_*_valid is asserted only when an update reaches the
                    -- write stage. Treat an older matching descriptor anywhere
                    -- in the ordered RMW pipeline as future-valid too. By the
                    -- time this update reaches the read stage, that predecessor
                    -- is either covered by the existing add/sum/write forwarding
                    -- mux above or has committed to RAM. This closes the window
                    -- where a captured zero validity bit discarded the committed
                    -- predecessor while preserving one accepted update per clock.
                    if ((upd_issue_valid = '1') and
                        (upd_issue_bank = i_upd_bank) and
                        (upd_issue_bin = i_upd_bin)) or
                       ((upd_read_valid = '1') and
                        (upd_read_bank = i_upd_bank) and
                        (upd_read_bin = i_upd_bin)) or
                       ((upd_add_valid = '1') and
                        (upd_add_bank = i_upd_bank) and
                        (upd_add_bin = i_upd_bin)) or
                       ((upd_sum_valid = '1') and
                        (upd_sum_bank = i_upd_bank) and
                        (upd_sum_bin = i_upd_bin)) or
                       ((upd_write_valid = '1') and
                        (upd_write_bank = i_upd_bank) and
                        (upd_write_bin = i_upd_bin)) then
                        ram_v_update_old_valid := '1';
                    end if;
