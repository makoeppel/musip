...
port (
  ...
  --Firefly_LPM                 : out   std_logic;-- Firefly Low Power Mode: Modules power consumption should be below 1.5 W. active high. Overrideable by I2C commands. Override default: high power (page 19 of documentation).
    ...
    FPGA_Test                   : out   std_logic_vector(7 downto 0) := (others => '0');

    -- LCD (2.5V)
    lcd_csn                     : out   std_logic; -- LCD Chip Select
    lcd_d_cn                    : out   std_logic; -- LCD Data / Command Select
    lcd_data                    : out   std_logic_vector(7 downto 0) := (others => '0');
    lcd_wen                     : out   std_logic; -- LCD Write Enable
...
--------------------------------------------------------------------
    ...
    -- TODO: we should re-work the reset in the future
    SIN_A <= '1' when run_state_125_reg = RUN_STATE_SYNC else mp_ctrl_SIN(0);
    SIN_B <= '1' when run_state_125_reg = RUN_STATE_SYNC else mp_ctrl_SIN(1);
    SIN_C <= '1' when run_state_125_reg = RUN_STATE_SYNC else mp_ctrl_SIN(2);
    SIN_D <= '1' when run_state_125_reg = RUN_STATE_SYNC else mp_ctrl_SIN(3);
    FPGA_Test(3) <= '1' when run_state_125_reg = RUN_STATE_SYNC else mp_ctrl_SIN(0);
    FPGA_Test(4) <= '1' when run_state_125_reg = RUN_STATE_SYNC else mp_ctrl_SIN(1);
    FPGA_Test(5) <= '1' when run_state_125_reg = RUN_STATE_SYNC else mp_ctrl_SIN(2);
    FPGA_Test(6) <= '1' when run_state_125_reg = RUN_STATE_SYNC else mp_ctrl_SIN(3);
