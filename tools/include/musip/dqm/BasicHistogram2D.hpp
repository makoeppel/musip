...
class BasicHistogram2D {
public:
    using xaxis_type = xaxis_type_;
    using yaxis_type = yaxis_type_;
    using content_type = content_type_;
    using x_axis_type = boost::histogram::axis::regular<xaxis_type>;
    using y_axis_type = boost::histogram::axis::regular<yaxis_type>;
...
private:
    ...
    xaxis_type lowXEdge_;
    xaxis_type highXEdge_;
    yaxis_type lowYEdge_;
    yaxis_type highYEdge_;
...
public:
    ...
    BasicHistogram2D(std::mutex* pMutex, unsigned numberOfXBins, xaxis_type lowXEdge, xaxis_type highXEdge, unsigned numberOfYBins, yaxis_type lowYEdge, yaxis_type highYEdge, metadata_parameters&&... metadataParams)
        : mutex_(pMutex),
          xAxis_(numberOfXBins, lowXEdge, highXEdge),
          yAxis_(numberOfYBins, lowYEdge, highYEdge),
          data_((numberOfXBins + additional_bins) * (numberOfYBins + additional_bins)),
          entries_(0),
          lowXEdge_(lowXEdge),
          highXEdge_(highXEdge),
          lowYEdge_(lowYEdge),
          highYEdge_(highYEdge),
    ...
    void add(const BasicHistogram2D& other, std::error_code& error);

    template<Lock lock = Lock::PerformLock, Lock otherLock = Lock::PerformLock>
    void add(BasicHistogram2D&& other, std::error_code& error);
    ...
    using root_type = typename detail::root_type<2, content_type>::type;
...
void musip::dqm::BasicHistogram2D<xaxis_type_, yaxis_type_, content_type_>::fill(xaxis_type xValue, yaxis_type yValue, content_type weight) {
    typename detail::guard_type<lock, mutex_type>::type lockGuard(mutex_);

    const int boostXIndex = xAxis_.index(xValue);
    const int boostYIndex = yAxis_.index(yValue);

    data_[ (boostXIndex + bin_offset) + (xAxis_.size() + additional_bins) * (boostYIndex + bin_offset)] += weight;
    ++entries_;
}

template<typename xaxis_type_, typename yaxis_type_, typename content_type_>
template<musip::dqm::Lock lock>
size_t musip::dqm::BasicHistogram2D<xaxis_type_, yaxis_type_, content_type_>::entries() const {
    typename detail::guard_type<lock, mutex_type>::type lockGuard(mutex_);
...
template<typename xaxis_type_, typename yaxis_type_, typename content_type_>
unsigned musip::dqm::BasicHistogram2D<xaxis_type_, yaxis_type_, content_type_>::numberOfXBins() const {
...
template<typename xaxis_type_, typename yaxis_type_, typename content_type_>
xaxis_type_ musip::dqm::BasicHistogram2D<xaxis_type_, yaxis_type_, content_type_>::lowXEdge() const {
    // Note that we don't lock the mutex for this, because once the histogram has been created
    // the binning can't change.
    return lowXEdge_;
}
...
template<typename xaxis_type_, typename yaxis_type_, typename content_type_>
xaxis_type_ musip::dqm::BasicHistogram2D<xaxis_type_, yaxis_type_, content_type_>::highXEdge() const {
    // Note that we don't lock the mutex for this, because once the histogram has been created
    // the binning can't change.
    return highXEdge_;
}
...
template<typename xaxis_type_, typename yaxis_type_, typename content_type_>
unsigned musip::dqm::BasicHistogram2D<xaxis_type_, yaxis_type_, content_type_>::numberOfYBins() const {
    // Note that we don't lock the mutex for this, because once the histogram has been created
    // the binning can't change.
    return yAxis_.size();
}
...
template<typename xaxis_type_, typename yaxis_type_, typename content_type_>
yaxis_type_ musip::dqm::BasicHistogram2D<xaxis_type_, yaxis_type_, content_type_>::lowYEdge() const {
    // Note that we don't lock the mutex for this, because once the histogram has been created
    // the binning can't change.
    return lowYEdge_;
}
...
template<typename xaxis_type_, typename yaxis_type_, typename content_type_>
yaxis_type_ musip::dqm::BasicHistogram2D<xaxis_type_, yaxis_type_, content_type_>::highYEdge() const {
    // Note that we don't lock the mutex for this, because once the histogram has been created
    // the binning can't change.
    return highYEdge_;
}
...
void musip::dqm::BasicHistogram2D<xaxis_type_, yaxis_type_, content_type_>::add(const BasicHistogram2D& other, std::error_code& error) {
    return addImpl<lock, otherLock>(other, error);
}

template<typename xaxis_type_, typename yaxis_type_, typename content_type_>
template<musip::dqm::Lock lock, musip::dqm::Lock otherLock>
void musip::dqm::BasicHistogram2D<xaxis_type_, yaxis_type_, content_type_>::add(BasicHistogram2D&& other, std::error_code& error) {
    return addImpl<lock, otherLock>(std::move(other), error);
}

template<typename xaxis_type_, typename yaxis_type_, typename content_type_>
template<musip::dqm::Lock lock>
void musip::dqm::BasicHistogram2D<xaxis_type_, yaxis_type_, content_type_>::add(const content_type* data, size_t dataSize, size_t entries, std::error_code& error) {
...
template<typename xaxis_type_, typename yaxis_type_, typename content_type_>
template<musip::dqm::Lock lock>
void musip::dqm::BasicHistogram2D<xaxis_type_, yaxis_type_, content_type_>::add(const content_type* data, size_t dataSize, std::error_code& error) {
...
template<typename xaxis_type_, typename yaxis_type_, typename content_type_>
template<musip::dqm::Lock lock>
void musip::dqm::BasicHistogram2D<xaxis_type_, yaxis_type_, content_type_>::clear() {
    typename detail::guard_type<lock, mutex_type>::type lockGuard(mutex_);
...
template<typename xaxis_type_, typename yaxis_type_, typename content_type_>
template<musip::dqm::Lock lock>
std::unique_ptr<typename musip::dqm::BasicHistogram2D<xaxis_type_, yaxis_type_, content_type_>::root_type> musip::dqm::BasicHistogram2D<xaxis_type_, yaxis_type_, content_type_>::asRootObject(const std::string& histogramName, const std::string& histogramTitle) const {
    typename detail::guard_type<lock, mutex_type>::type lockGuard(mutex_);
    ...
    auto pHistogram = std::make_unique<root_type>(histogramName.c_str(), title.c_str(), numberOfXBins(), lowXEdge(), highXEdge(), numberOfYBins(), lowYEdge(), highYEdge());
    ...
    const std::string& axisTitleZ = metadata_.get<Metadata::Category::AxisTitleZ>();
    if(&axisTitleZ != &Metadata::nullEntry) pHistogram->GetZaxis()->SetTitle(axisTitleZ.c_str());
...
void musip::dqm::BasicHistogram2D<xaxis_type_, yaxis_type_, content_type_>::addImpl(histogram_type&& other, std::error_code& error) {
    ...
    if(numberOfXBins() != other.numberOfXBins() || lowXEdge() != other.lowXEdge() || highXEdge() != other.highXEdge()
        || numberOfYBins() != other.numberOfYBins() || lowYEdge() != other.lowYEdge() || highYEdge() != other.highYEdge()) {
