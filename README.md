...
Current validation status on May 10, 2026:

- the full Siemens Questa install at `/data1/questaone_sim/questasim` is the only supported simulator on this host,
- `mu3e-ip-cores` is tracked in-repo as the pull-only `external/mu3e-ip-cores` consumer snapshot and is the default upstream owner for OPQ packaging and sync; the OPQ Qsys wrapper is regenerated from packet_scheduler `26.5.1.0509` at nested commit `245eb93` through parent snapshot `4603656`, which also pins mutrig_timestamp_processor `26.0.13.0510` at `9692869`,
...
- `make ip-ghdl-cross-run` plus `make ip-ghdl-cross-checkpoints` is the fast local GHDL/GTKWave inspection path for packet-level OPQ evidence; it is debug evidence, not a replacement for the promoted UVM/UCDB signoff runs,
...
1. Run `make ip-init`
2. Run `make ip-svd`
3. Run `make ip-csr-lint`
4. Run `make ip-check-license`
5. Run `make ip-tlm-basic-smoke`
6. Run `make ip-compile-basic`
7. Run `make ip-uvm-basic SIM_ARGS="+SWB_REPLAY_DIR=$(pwd)/tb_int/cases/basic/ref/out_smoke"`
8. Run `make ip-tlm-basic`
9. Run `make ip-plain-basic`
10. Run `make ip-plain-basic-2env`
11. Run `make ip-uvm-longrun`
12. Run `make ip-cross-baselines`
13. Run `make ip-ghdl-cross-run`
14. Run `make ip-ghdl-cross-checkpoints`
15. Run `CLOSURE_RESUME=1 make ip-cov-closure`
...
- `make ip-ghdl-cross-objects`, `make ip-ghdl-cross-run`, `make ip-ghdl-cross-gtkw`, `make ip-ghdl-cross-checkpoints`, and `make ip-ghdl-cross-view` live under `tb_int/cases/cross/ghdl/`. They build a deterministic GHDL waveform fixture with decoded packet streams, OPQ dataflow/fill-level groups, case markers, and named checkpoint validation for quick visual triage.
- The exact native-SV-vs-GHDL waveform comparison flow is documented in `tb_int/cases/cross/ghdl/README.md`; it exports a 3-frame `B001` native-SV UVM VCD, extracts the UVM ingress/egress packet streams into the GHDL replay, and compares both the critical SOP/EOP timing points and packet integrity: grammar, `0x800` timestamp steps, OPQ count fields, and ingress-to-OPQ hit/subframe-hit multisets.
