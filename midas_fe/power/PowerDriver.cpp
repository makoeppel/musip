...
INT PowerDriver::Connect() {
    ...
    if (conn_type == "Tcp") {
        ...
        client = new SerialClient(usbp, settings["Baudrate"], settings["Character size"], parit, settings["Stop Bits"], 2000, flw_ctrl);
    } else {
...
float PowerDriver::Read(std::string cmd, INT& error) {
    ...
    try {
        if (reply.empty()) {
            cm_msg(MERROR, "Power supply read...", "no reply is given as a result of command %s.", cmd.c_str());
            error = FE_ERR_DRIVER;
        } else if (reply.find("smu") != std::string::npos) {
            reply = reply.substr(12, 14);
            value = std::stof(reply);
        } else if ((reply.find_first_of("eE") != std::string::npos) || (classname.find("Keithley") != std::string::npos)) {
...
std::vector<std::string> PowerDriver::ReadErrorQueue(int index, INT& error) {
    ...
    while(1) {
        ...
        if (cmd.empty() && (classname.find("Keithley") != std::string::npos)) { break; }
...
WORD PowerDriver::ReadQCGE(int index, INT& error) {
    ...
    std::string reply("");
    if (!client->ReadReply(&reply, min_reply_length)) {
        cm_msg(MERROR, "Power supply read ... ", "could not read QCGE supply with address %d", instrumentID[index]);
        error = FE_ERR_DRIVER;
    }
...
bool PowerDriver::ReadState(int index, INT& error) {
    ...
    std::string reply("");
    if (!client->ReadReply(&reply, min_reply_length)) {
        cm_msg(MERROR, "Power supply read ... ", "could not read off %s state supply/channel: %d.", name.c_str(), instrumentID[index]);
        error = FE_ERR_DRIVER;
    }
...
std::string PowerDriver::ReadSourceMode(int index, INT& error) {
    ...
    std::string reply("");
    if (!client->ReadReply(&reply, min_reply_length)) {
        cm_msg(MERROR, "Power supply read ... ", "could not read off %s source mode supply/channel: %d.", name.c_str(), instrumentID[index]);
        error = FE_ERR_DRIVER;
    }
...
bool PowerDriver::Set(std::string cmd, INT& error) {
    ...
    if (OPC()) {
        ...
        error = FE_ERR_DRIVER;
        cm_msg(MERROR, "Power supply ... ", "command %s not succesful for %s supply", cmd.c_str(), name.c_str());
        return false;
    }
}

void PowerDriver::SetCurrentLimit(int index, float value, INT& error) {
    // Have to lock it here (and not in the Set() function), otherwise you might change the wrong channel
    ...
    if (SelectChannel(instrumentID[index])) {
        bool success(true);
        if (classname == "Keithley2400" || classname == "Keithley2450" || classname == "Keithley6487") {
            success = Set(GenerateCommand(COMMAND_TYPE::SetCurrentLimit, value), error);
        } else {
            cm_msg(MINFO, "Power ... ", "changing current limit of %s, channel %d requested, command = %d.", name.c_str(), instrumentID[index], COMMAND_TYPE::SetCurrent);
        ...
        if (success) {
            ...
            // voltage[index] = ReadVoltage(index,error); // These functions also have a lock_guard, so can not be called directly
...
void PowerDriver::SetVoltage(int index, float value, INT& error) {
    ...
    if (classname == "Keithley2450") {
        ...
        std::string sourceMode(settings["Source Mode"]);
        if (sourceMode == "CURR") {
            cm_msg(MERROR, "Power supply ... ", "voltage change is not allowed for this device given the source mode.");
    ...
    if ((classname.find("Keithley6487") != std::string::npos) // a Keithley6487 is a voltage source at UZH
        && (value > 0)
    ...
    if (SelectChannel(instrumentID[index])) { // module address in the daisy chain to select channel, or 1/2/3/4 for the HAMEG
        if (Set(GenerateCommand(COMMAND_TYPE::SelectChannelAndSetVoltage, instrumentID[index], value), error)) {
            readonlythisindex = index;
...
void PowerDriver::SetCurrent(int index, float value, INT& error) {
    ...
    if (classname == "Keithley2450") {
        ...
        std::string sourceMode(settings["Source Mode"]);
        if (sourceMode == "VOLT") {
            cm_msg(MERROR, "Power supply ... ", "current change is not allowed for this device given the source mode.");
    ...
    if ((classname.find("Keithley") == std::string::npos)
        ...
        // && (classname.find("Keithley") != std::string::npos) // Now the Keithley2450 is setting current at UZH
    ) {
        cm_msg(MERROR, "Power supply ... ", "current change is not allowed for this device, revisit your strategy.");
        variables["Demand Current"][index] = demandcurrent[index]; // Disable request
        error = FE_ERR_DISABLED;
        return;
    }
    ...
    if (SelectChannel(instrumentID[index])) { // module address in the daisy chain to select channel, or 1/2/3/4 for the HAMEG
        if (Set(GenerateCommand(COMMAND_TYPE::SelectChannelAndSetCurrent, instrumentID[index], value), error)) {
            readonlythisindex = index;
            // voltage[index] = ReadVoltage(index, error);
            // variables["Voltage"][index] = voltage[index];
            // current[index] = ReadCurrent(index, error);
...
void PowerDriver::SetState(int index, bool value, INT& error) {
    ...
    cm_msg(MINFO, "Power supply ... ", "request to set channel %d to %d", instrumentID[index], value);

    if (value && !AskPermissionToTurnOn(index)) {
        cm_msg(MERROR, "Power supply ... ", "changing the state of channel %d not allowed, name = %s.", instrumentID[index], name.c_str());
...
void PowerDriver::CurrentLimitChanged() {
    ...
    for (unsigned int i(0); i < currentlimit.size(); i++) {
        ...
        if (fabs(value - currentlimit[i]) > fabs(relevantchange * currentlimit[i]) ) { // Compare to local book keeping, look for significant change
            ...
            if (err == FE_SUCCESS) {
                cm_msg(MINFO, "Power supply ... ", "changing %s current limit of channel %d to %f", name.c_str(), instrumentID[i], value);
                nChannelsChanged++;
                currentlimit[i] = value;
            } else {
                variables["Current Limit"][i] = currentlimit[i]; // Set back to local book keeping
                cm_msg(MERROR, "Power supply ... ", "changing %s current limit of channel %d to %f failed, error %d", name.c_str(), instrumentID[i], value, err);
            }
        }
    }
    if (nChannelsChanged < 1) { cm_msg(MINFO, "Power supply ... ", "changing current limit request of %s rejected", name.c_str()); }
}
...
void PowerDriver::SetStateChanged() {
    ...
    for (unsigned int i(0); i < state.size(); i++) {
        ...
        if (value != state[i]) { // Compare to local book keeping
            ...
            if (err == FE_SUCCESS) {
                cm_msg(MINFO, "Power supply ... ", "changing %s state of channel %d to %d", name.c_str(), instrumentID[i], value);
            } else {
                variables["Set State"][i] = state[i]; // Set back to local book keeping
                cm_msg(MERROR, "Power supply ... ", "changing %s state of channel %d to %d failed, error %d", name.c_str(), instrumentID[i], value, err);
            }
        }
    }
...
void PowerDriver::DemandVoltageChanged() {
    ...
    for (unsigned int i(0); i < demandvoltage.size(); i++) {
        ...
        if (fabs(value - demandvoltage[i]) > fabs(relevantchange * demandvoltage[i]) ) { // Compare to local book keeping, look for significant change
            SetVoltage(i, value, err);
            if (err == FE_SUCCESS) {
                cm_msg(MINFO, "Power supply ... ", "changing %s voltage of channel %d to %f", name.c_str(), instrumentID[i], value);
                nChannelsChanged++;
                demandvoltage[i] = value;
            } else {
                variables["Demand Voltage"][i] = demandvoltage[i]; // Set back to local book keeping
                cm_msg(MERROR, "Power supply ... ", "changing %s voltage of channel %d to %f failed, error %d", name.c_str(), instrumentID[i], value, err);
            }
        }
    }
    if (nChannelsChanged < 1) { cm_msg(MINFO, "Power supply ... ", "changing voltage request of %s rejected", name.c_str()); }
}
...
void PowerDriver::DemandCurrentChanged() {
    ...
    for (unsigned int i(0); i < demandcurrent.size(); i++) {
        ...
        cm_msg(MINFO, "Power supply ... ", "Trying to change %s current of channel %d to %f", name.c_str(), instrumentID[i], value);
        if (fabs(value - demandcurrent[i]) > fabs(relevantchange * demandcurrent[i]) ) { // Compare to local book keeping, look for significant change
            cm_msg(MINFO, "Power supply ... ", "Setting %s current of channel %d to %f", name.c_str(), instrumentID[i], value);
            SetCurrent(i, value, err);
            if (err == FE_SUCCESS) {
                cm_msg(MINFO, "Power supply ... ", "changing %s current of channel %d to %f", name.c_str(), instrumentID[i], value);
                nChannelsChanged++;
                demandcurrent[i] = value;
            } else {
                variables["Demand Current"][i] = demandcurrent[i]; // Set back to local book keeping
                cm_msg(MERROR, "Power supply ... ", "changing %s current of channel %d to %f failed, error %d", name.c_str(), instrumentID[i], value, err);
            }
        }
    }
    if (nChannelsChanged < 1) { cm_msg(MINFO, "Power supply ... ", "changing current request of %s rejected", name.c_str()); }
}
...
void PowerDriver::DemandOVPLevelChanged() {
    ...
    for (unsigned int i(0); i < OVPlevel.size(); i++) {
        ...
        if (fabs(value - OVPlevel[i]) > fabs(relevantchange * OVPlevel[i]) ) { // Compare to local book keeping, look for significant change
            ...
            if (err == FE_SUCCESS) {
                cm_msg(MINFO, "Power supply ... ", "changing %s voltage protection level of channel %d to %f", name.c_str(), instrumentID[i], value);
                nChannelsChanged++;
                OVPlevel[i] = value;
                variables["OVP Level"][i] = value;
            } else {
                variables["Demand OVP Level"][i] = OVPlevel[i]; // Set back to local book keeping
                cm_msg(MERROR, "Power supply ... ", "changing %s voltage protection level of channel %d to %f failed, error %d", name.c_str(), instrumentID[i], value, err);
            }
        }
    }
    if (nChannelsChanged < 1) { cm_msg(MINFO, "Power supply ... ", "changing voltage protection level request of %s rejected", name.c_str()); }
}
