...
static void ida_free(struct ida* ida, unsigned int id) { ida_simple_remove(ida, id); }

static int ida_alloc(struct ida* ida, gfp_t gfp) { return ida_alloc_range(ida, 0, ~0, gfp); }
#endif
...
static
    ...
    int
    dmabuf_entry_cmp(void* priv, const struct list_head* a, const struct list_head* b) {
#else
    int
    dmabuf_entry_cmp(void* priv, struct list_head* a, struct list_head* b) {
#endif
 ...
 */
static int dmabuf_report(struct dmabuf* dmabuf) {
    int n = 0;
    ...
    list_for_each_entry(entry, &dmabuf->entries, list_head) {
        ...
        while (!list_is_last(&entry->list_head, &dmabuf->entries)) {
            ...
            if (dma_handle + size != next->dma_handle)
                break;
            size += next->size;
            entry = next;
        }
...
static void dmabuf_free(struct dmabuf* dmabuf) {
 ...
 */
static struct dmabuf* dmabuf_alloc(struct device* dev, size_t size) {
    int error;
    ...
    retry_alloc:
        ...
        entry->cpu_addr =
            dma_alloc_coherent(dmabuf->dev, entry->size, &entry->dma_handle,
                               GFP_ATOMIC | __GFP_NOWARN);  // see `pci_alloc_consistent`
...
static loff_t dmabuf_llseek(struct dmabuf* dmabuf, struct file* file, loff_t loff, int whence) {
    loff_t loff_new;
 ...
 */
static int dmabuf_mmap(struct dmabuf* dmabuf, struct vm_area_struct* vma) {
    int error;
    ...
    if (offset > dmabuf->size)
        return -EINVAL;
    if (vma_size > dmabuf->size - offset)
        return -EINVAL;
    ...
    list_for_each_entry(entry, &dmabuf->entries, list_head) {
        ...
        unsigned long pfn = PHYS_PFN(
            dma_to_phys(dmabuf->dev, entry->dma_handle + offset));  // see `dma_direct_mmap`
...
static ssize_t dmabuf_read(struct dmabuf* dmabuf, char __user* user_buffer, size_t user_size,
                           loff_t offset) {
    ssize_t n = 0;
    struct dmabuf_entry* entry;
...
static ssize_t dmabuf_write(struct dmabuf* dmabuf, const char __user* user_buffer, size_t user_size,
                            loff_t offset) {
    ssize_t n = 0;
    struct dmabuf_entry* entry;
