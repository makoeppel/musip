...
static loff_t dmabuf_fops_llseek(struct file* file, loff_t loff, int whence) {
    struct dmabuf* dmabuf = file->private_data;
    return dmabuf_llseek(dmabuf, file, loff, whence);
}

static ssize_t dmabuf_fops_read(struct file* file, char __user* user_buffer, size_t size,
                                loff_t* offset) {
    struct dmabuf* dmabuf = file->private_data;
...
static ssize_t dmabuf_fops_write(struct file* file, const char __user* user_buffer, size_t size,
                                 loff_t* offset) {
    struct dmabuf* dmabuf = file->private_data;
...
static int dmabuf_fops_mmap(struct file* file, struct vm_area_struct* vma) {
    struct dmabuf* dmabuf = file->private_data;
    return dmabuf_mmap(dmabuf, vma);
}

static int dmabuf_fops_release(struct inode* inode, struct file* file) {
    M_INFO("\n");
