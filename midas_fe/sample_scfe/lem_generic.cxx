...
typedef struct {
   ...
   DWORD last_change; // last change time stamp
...
INT gen_read(EQUIPMENT * pequipment, int channel)
{
   ...
   for (i = 0; i < gen_info->num_channels; i++) {
      // update if change is more than update_threshold or update timeout occured
      ...
      if ((ss_isnan(gen_info->measured[i]) && !ss_isnan(gen_info->measured_mirror[i])) ||
          ...
          (act_time - gen_info->last_change > UPDATE_TIMEOUT)) {
         gen_info->last_change = act_time;
         for (i = 0; i < gen_info->num_channels; i++)
...
INT gen_init(EQUIPMENT * pequipment)
{
   ...
   if (!gen_info->driver) {
      cm_msg(MERROR, "gen_init", "Not enough memory");
      return FE_ERR_ODB;
   }
