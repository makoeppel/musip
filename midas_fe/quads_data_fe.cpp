...
int begin_of_run()
{
        ...
        if(m_settings["Readout"]["Datagen Enable"]) {
            ...
            cm_msg(MINFO, "quad_fe", "Use datagenerator with divider register %i", (int) stream_settings["Datagen Divider"]);
    ...
    n_mevents = m_settings["Readout"]["n_mevents"];
        ...
        mu.write_register(GET_N_DMA_WORDS_REGISTER_W, m_settings["Readout"]["max_requested_words"] / (256/32));
...
int create_midas_events(uint32_t * dmaBuffer, uint32_t dmaBufSize, int rbh, uint32_t idx) {
    ...
    while (bank_offset - idx < bh->data_size / 4) {
        ...
        if ( (bank_data_end-bank_data_start) > 16384 ) {
            cm_msg(MERROR, "create_midas_events()", "Event size %i bytes is bigger then max RAM size 16384 bytes on FPGA!\n", bank_data_end-bank_data_start);
            break;
        }
    ...
    if ( mevents[ts].size() > 10 ) {
        cm_msg(MERROR, "create_midas_events()", "mevents.size() = %d and mevents[ts].size() = %d > 10\n", int(mevents.size()), int(mevents[ts].size()));
    }
    ...
    for ( int i = 0; mevents.size() > 1 && i < 100; i++ ) {
        ...
        { // - write *HIT bank
            char* data = nullptr;
            std::string bank_name = mevents.begin()->second.front().hits_name.substr(2, 2) + fmt::format("{:02d}", i);
            bk_create(bankHeader, bank_name.c_str(), TID_UINT32, reinterpret_cast<void**>(&data));
            for(auto& mevent : mevents.begin()->second) {
...
int read_stream_thread(void *)
{
    ...
    while(is_readout_thread_enabled()) {
        ...
        if (use_software_dummy) {
            ...
            serial_number = create_dummy_event(dma_buf_dummy.get(), eventSize, nEvents, serial_number);
...
EQUIPMENT equipment[] = {
    {"Quads Data",                     /* equipment name */
        {eventID_data, 0,              /* event ID, trigger mask */
            ...
            "MIDAS",                   /* format */
            TRUE,                      /* enabled */
            RO_RUNNING,                /* read always, except during transistions and update ODB */
